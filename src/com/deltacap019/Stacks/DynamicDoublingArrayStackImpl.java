 ...
 * Array will grow by doubling itself once it is full and we will copy the elements from previous array to new array.
 * We will create an array of capacity 1 and thereafter we will implement PUSH and POP
 * operations on it.
 * LEss copy operations performed here which reduces the over all complexity in trade of space.
 * We consider Amortized Time for it whihc comes to O(1). for insertion.
 */

public class DynamicDoublingArrayStackImpl {

    private int[] stack;
    private int top = -1; // stores current index of tha array.

    public DynamicDoublingArrayStackImpl() {
        stack = new int[1];
    }
